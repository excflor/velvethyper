...
export function registerVmxHandlers(ipcMain: IpcMain): void {
  ...
  ipcMain.handle('harden-vm', async (_, vmxPath: string, profile: any) => {
    ...
    if (!profile)
      return {
        success: false,
        message: 'No identity profile selected. Generate an identity first.'
      }
    ...
    console.log(
      `[Main] Hardening VM: ${vmxPath} — Profile: ${profile.manufacturer} / ${profile.model}`
    )
  ...
  ipcMain.handle('check-vmx-status', async (_, vmxPath: string) => {
    ...
    try {
      const { stdout } = await execAsync(`python "${hardenScript}" --check "${vmxPath}"`, {
